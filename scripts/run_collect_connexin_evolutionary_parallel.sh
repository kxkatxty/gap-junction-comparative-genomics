#!/usr/bin/env bash
# Sample 5 unannotated-connexin species per evolutionary clade, in parallel with SynVoy.
set -uo pipefail
source "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/_env.sh"
PARTS="${ROOT}/project/metadata/connexin_evolutionary_parts"
LOG_DIR="${ROOT}/project/results/connexin_evolutionary_collect/logs"
CATALOG="${ROOT}/project/metadata/species_clade_catalog_connexin_evolutionary.csv"
QUEUE="${ROOT}/project/metadata/not_annotated_connexin_evolutionary.txt"
STATUS="${ROOT}/project/metadata/family_annotation_status_connexin_evolutionary.csv"
...
catalog_out = root / "project/metadata/species_clade_catalog_connexin_evolutionary.csv"
if catalog_rows:
...
queue_out = root / "project/metadata/not_annotated_connexin_evolutionary.txt"
queue_out.write_text("\n".join(queue) + ("\n" if queue else ""), encoding="utf-8")

status_out = root / "project/metadata/family_annotation_status_connexin_evolutionary.csv"
if status_rows:
